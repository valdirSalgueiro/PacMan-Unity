...
namespace RoyT.AStar
{
    ...
    public sealed class Grid
    {
        ...
        public void SetCellCost(Position position, float cost)
        {
            if (cost < 1)
            {
                throw new ArgumentOutOfRangeException("Argument {nameof(cost)} with value {cost} is invalid. The cost of traversing a cell cannot be less than one");
            }
        ...
        private int GetIndex(int x, int y)
        {
            if (x < 0 || x >= this.DimX)
            {
                throw new ArgumentOutOfRangeException("The x-coordinate {x} is outside of the expected range [0...{this.DimX})");
            }

            if (y < 0 || y >= this.DimY)
            {
                throw new ArgumentOutOfRangeException("The y-coordinate {y} is outside of the expected range [0...{this.DimY})");
            }
