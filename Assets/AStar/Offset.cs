...
namespace RoyT.AStar
{
    ...
    public struct Offset : IEquatable<Offset>
    {
        ...
        {
            if (x < -1 || x > 1)
                throw new ArgumentOutOfRangeException("Parameter {nameof(x)} cannot have a magnitude larger than one");

            if (y < -1 || y > 1)
                throw new ArgumentOutOfRangeException("Parameter {nameof(y)} cannot have a magnitude larger than one");

            if (x == 0 && y == 0)
                throw new ArgumentException("Paramters {nameof(x)} and {nameof(y)} cannot both be zero");
